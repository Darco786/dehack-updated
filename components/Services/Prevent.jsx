...
import Meta from '@/public/prevent.json'

const Prevent = ({ image, IBM, serviceTitle, shortDesc, desc }) => {
  ...
  return (
    ...
    <div className='gradient-border !rounded-[15px] md:!rounded-[7px] md:min-w-[300px] relative z-10 p-6 md:px-12 2xl:px-20 hover:border-white transitions !border-2 md:!border-[3px]'>
        <div className='flex flex-col md:flex-row gap-6 g:gap-10 md:items-center mb-4'>
        <Lottie
        ...
        <div className='flex flex-col gap-1 2xl:gap-2'>
            <h4 className={`font-bold text-4xl md:text-xl lg:text-3xl 2xl:text-[36px] ${IBM?.className}`}>{serviceTitle}</h4>
            <p className='text-lg 2xl:text-xl'>{shortDesc}</p>
        </div>
        </div>
        <p className='text-sm md:text-base 2xl:text-lg font-light spacing'>{desc}</p>
